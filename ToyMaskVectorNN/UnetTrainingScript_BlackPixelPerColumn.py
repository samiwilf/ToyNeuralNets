...
def random_sample_generator(x_dir, y_dir, image_names, batch_size, bit_depth, dim1, dim2, rescale_labels):
    ...
    while True:
        ...
        xnp = np.zeros((batch_size, 1024, columnCount, x_channels), dtype=np.float32)        
        ynp = np.zeros((batch_size, 1, columnCount, y_channels), dtype=np.float32)
        ...
        for batchInx in range(batch_size):
            ...
            for x in range(columnCount):
                ...
                xnp[batchInx, y, x, 0] = 1.0
                ynp[batchInx, 0, x, 0] = y

                for y in range(1024):
                    xnp[batchInx, y, x, 1] = x
                    xnp[batchInx, y, x, 2] = y
        ...
        #x *= ifactor
        x = np.reshape(xnp, (batch_size, 1024, columnCount, 3))
        ...
        yield(x, np.reshape(ynp, (batch_size, columnCount)))
...
def get_core(dim1, dim2):
    ...
    y = Conv2D(3, kernel_size = (1,1), **option_dict_conv)(x)  
    return [x,y]
...
def get_model(dim1, dim2, activation="relu"):
    ...
    print(y.shape)
    #y = keras.layers.SeparableConv1D(64, kernel_size = 1024, **{"activation": "relu"} )(y)

    #y = keras.layers.Conv2D(1024, kernel_size = (1, 1024), **{"activation": "relu"} )(y)
    #y = keras.layers.Conv2D(1024, kernel_size = (1, 1024), **{"activation": "relu"} )(y)
    #y = keras.layers.Conv2D(1024, kernel_size = (1, 1024), **{"activation": "relu"} )(y)
...
#######################################################################################
#######################################################################################
optimizer =  keras.optimizers.RMSprop(lr=0.00100101)
