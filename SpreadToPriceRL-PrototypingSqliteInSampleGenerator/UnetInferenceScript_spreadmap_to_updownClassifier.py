...
def random_sample_generator(x_dir, y_dir, image_names, batch_size, bit_depth, dim1, dim2, rescale_labels):
    ...
    while True:
        ...
        for batchInx in range(batch_size):
            ...
            blob = record[2]
            envbuffer_orig = np.frombuffer(blob, dtype=np.float32)

            envbuffer = np.reshape(envbuffer_orig[:-3], (2048 + 1 + 4, 256))
            ...
            print("\nenvbuffer_orig[-3]")
            print(envbuffer_orig[-3])
            print("\n")

            x[batchInx, 0:2048, 0:128, 0] = envbuffer[0:2048,128:256]
            y[batchInx, int(envbuffer_orig[-3])] = 1
...
def weighted_crossentropy(y_true, y_pred):
    ...
    weights = 1#tf.reduce_sum(class_weights * y_true)
...
for y_e in y:
    y_out.append([])
    for y_ee in y_e:
        y_out[-1].append(f"{y_ee:.9f}")

pred_out = []
for y_e in predictions:
    pred_out.append([])
    for y_ee in y_e:
        pred_out[-1].append(f"{y_ee:.9f}")

for i in range(len(y_out)):
    for j in range( len(y_out[i])):
        print(y_out[i][j], pred_out[i][j]) 
    print("\n")
#predictions_output = f"{predictions:.9f}"

#print(y_out, pred_out)
"""
